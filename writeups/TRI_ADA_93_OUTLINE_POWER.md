...
Mechanisms: to declare a hierarchy of tagged types (i.e. a class and subclasses), the associated types (class-wide types, access types) and operations.
Programming techniques: to understand the proper use of Ada's new mechanisms and to integrate them in the design of programs. A few advanced concepts will be examined in this context, including by-reference object semantics and mixin inheritance.
...
Introduction
Object-Oriented Programming with Ada 83
Mechanisms
Inheritance
Polymorphism
Genericity
...
Abstract Types
Programming Techniques
Variant Programming vs. Incremental Programming
Heterogeneous Data Structures
Combining Abstractions without Multiple Inheritance
Initialization, Duplication and Finalization
Software Components
...
5. Mechanisms
5.1. Inheritance
Inheritance = derivation + overriding + type extension + run-time type identification
...
One can program in terms of common properties. When calling a primitive operation for which several implementations are provided, the appropriate implementation is selected according to the specific type of the object. The selection can be made either at compile-time (static binding) or at run-time (dispatching-i.e. dynamic binding). In the latter case, selection is made according to the object's tag, which identifies its specific type and operations.

One can design class-wide subprograms. A class-wide subprogram is not bound to one specific type but to all types in a given class. Similarly, class-wide variables can hold an object of any specific type in a given class. Primitive operations are dispatching if called with class-wide actual parameters. Even though subprogram parameters are not always bound to exactly one type, there are no holes in the Ada type system; type-loss is impossible, since each object has a tag and keeps it; thus the safety of Ada's strong typing is preserved.
...
A compact tutorial outline that encodes an entire strategic position: make object orientation powerful enough for reuse, but never loose enough to abandon type safety.
